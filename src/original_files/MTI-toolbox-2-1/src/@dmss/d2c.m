function sys = d2c(sys)
% C2D - Convert discrete-time dynamic iMTI system to continuous-time.
%
%   SYSC = d2c(SYSD,TS) computes a continuous-time model with using one step
%   forward euler approximation.
%
% For detailed documentation see <a href="matlab:open((which('d2cDoc.html')))">here</a>
...
if isa(sys.ts,'double')
    if sys.ts == 0
        error('dmss model is already in continous-time!')
    end
end
...
H.phi.equality.c = [H.phi.equality.c, zeros(sys.nEq, 3*sys.n);...
                    zeros(sys.n, N), kron(eye(sys.n), [Ts 0 0])];
...
H.phi.equality.f = [H.phi.equality.f, zeros(sys.nEq, 3*sys.n);...
                    zeros(sys.n, N), kron(eye(sys.n), [0 0 1])];
