...
autoapi_root = 'api'
autoapi_add_toctree_entry = False   # gestionem el toctree manualment a index.rst
autoapi_options = [
    'members',
    'undoc-members',
    'show-inheritance',
...
_AUTOAPI_ENABLED = [
    ...
    'PolynomialMatrixBuilder.py',   # ← afegir aquí

]

# Construeix la llista d'exclusions: tot excepte els fitxers habilitats
import glob as _glob, os as _os
_all_py = [
    _os.path.basename(f)
    for f in _glob.glob(_os.path.join(_code_path, '*.py'))
] if _code_path else []
...
autoapi_keep_files = False              # regenera sempre en cada build
